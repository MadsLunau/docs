...
The agent's behavior can only change through a release you approve. Every configuration update the Internal Agent drafts is tested against real scenarios and drafted in [Actions](/platform/actions), where you review the exact diff before anything goes live. There's no path from suggestion to production that skips your team.
