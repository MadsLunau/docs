...
Every step the Internal Agent drives - build, test, identify gaps - happens in a task, and so does approving the release that puts a change in front of customers.
...
- **Building:** read and update your Customer Agent's configurations, and build tools and connections. See [Build](/internal-agent/build).
- **Testing:** run test simulations of your Customer Agent before changes ship. See [Test](/internal-agent/test).
- **Reporting:** analyze any slice of your tickets and generate reports, interactive graphs, and CSV files. See [Identify gaps](/internal-agent/identify-gaps).
- **Answering:** answer questions about your setup, your tickets, and how any ticket was handled.
- **Using connections:** act in your connected tools, like searching Linear for a known bug, posting to Slack, or updating a Notion page by building and running one-off coding scripts, for example to analyze ticket data at scale or pull data from a connected service like Stripe.
