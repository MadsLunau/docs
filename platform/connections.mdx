...
- **Linear:** Search, create, comment on, and update issues. Close the loop with customers when a bug is fixed.
- **GitHub:** Within connected repos, the agent reads PR and issue metadata: it can see a PR's subject, description, and codeline changes, but it can not access the general codebase.
- **Notion:** Read, create, and update pages. Keep your Customer Agent in sync with knowledge and policies.
- **Cursor:** Launch Cursor Cloud Agents against your codebase to verify reported bugs or open a PR. Connects support to engineering.
- **Slack:** Add any channel, search messages, post to channels or DMs, and tag **@parahelp** in a thread to start a task.
