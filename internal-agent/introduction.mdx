---
title: "Introduction"
description: "The Internal Agent builds, tests, releases, and identifies gaps for your Customer Agent."
---

The Internal Agent builds, tests, releases, and identifies gaps for your Customer Agent. Each step happens in a [task](/platform/tasks).

<CardGroup cols={2}>
  <Card title="Build" icon="hammer" href="/internal-agent/build">
    Describe what you want, and the Internal Agent builds it.
  </Card>

  <Card title="Test" icon="flask" href="/internal-agent/test">
    Simulate on your historical tickets before anything ships.
  </Card>

  <Card title="Release" icon="rocket" href="/internal-agent/release">
    Approve changes to publish them, versioned and revertable.
  </Card>

  <Card title="Identify gaps" icon="chart-line" href="/internal-agent/improve">
    The Customer Agent flags gaps, and automations propose the next update as your product changes.
  </Card>
</CardGroup>

For the full loop, start with [Build](/internal-agent/build).